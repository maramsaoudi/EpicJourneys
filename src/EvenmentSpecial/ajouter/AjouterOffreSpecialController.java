...
public class AjouterOffreSpecialController implements Initializable {
    ...
    void retourToSceneInitiale(ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("../main/GUIEVENMENTAdmin.fxml"));
    ...
    private void AjouterOffreSpeciale(ActionEvent event) {
        try {
            ...
            if (titreOffre.isEmpty() || prixOffreText.isEmpty() ||
                    guideIdOffreText.isEmpty()
                ...
                String niveauOffre = niveauOffreValue;
                System.out.println(niveauOffre);
                ...
                String categorieOffre = fxCattegorieOffre.getText();
                // Check if the selected date is in the past
                LocalDate currentDate = LocalDate.now();
                if (localDate.isBefore(currentDate)) {
                    fxAjouterOffrelabel.setText("Selected date is in the past.");
                } else {
                    OffreSpecialEvenment evenment1 = new OffreSpecialEvenment(categorieOffre, dateDepartOffre,
                            descriptionOffre, destinationOffre, guideIdOffre, imagePathOffre, prixOffre, titreOffre,
                            niveauOffre);
